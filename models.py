...
class ConvWrapper(nn.Module):
    ...
    def forward_anytime(self, x, threshold=0.9):
        """
        Forward `forward_anytime` through the conv backbone.
        Valid when head is ACTClassifier, PonderNetClassifier, or MultiExitMLP
        (any head exposing `forward_anytime(feats, threshold)`).
        """
        feats = self.backbone(x)
        return self.head.forward_anytime(feats, threshold=threshold)
