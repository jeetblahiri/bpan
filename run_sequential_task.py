...
class TransformerBaseline(nn.Module):
    """
    Transformer baseline for sequential tasks.
    Uses a causal self-attention encoder to process sequential inputs,
    producing predictions after each step (for per-step accuracy evaluation).
    """
    def __init__(self, input_dim, n_classes, hidden_dim=256,
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, 64, hidden_dim) * 0.02)

        # Transformer encoder with causal mask for autoregressive processing
    ...
    def _causal_mask(self, L, device):
        """Generate causal mask so position t can only attend to positions <= t."""
    ...
    def forward(self, x_seq, targets=None):
        ...
        h = self.input_proj(x_seq)  # [B, L, hidden_dim]
        h = h + self.pos_embedding[:, :L, :]

        causal_mask = self._causal_mask(L, device)
        h = self.transformer(h, mask=causal_mask)  # [B, L, hidden_dim]

        logits_seq = self.readout(h)  # [B, L, C]
...
def main():
    ...
    parser.add_argument("--model", type=str, default="bpan",
                        choices=["bpan", "gru", "lstm", "mlp", "transformer"])
